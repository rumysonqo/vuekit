...
<v-container fluid>
  <v-layout style="height:60px" mb-1>
    ...
    <h1 class="title">Clasificador</h1> 
    </v-flex>
    <v-flex xs6>
      <v-select
            v-model="cod_clf"
...
<script>
  ...
  export default {
    ...
    get_metas(){
        ...
        .then(r=>{
        this.ds_metas=r.data;
        console.log('metas:',this.ds_metas);
        })
        .catch(function(error){
        console.log(error);
        })
      ...
      get_clasif(){
        axios.get("http://localhost:3000/clasif/"+this.cod_met)
        .then(r=>{
        this.ds_clasif=r.data;
        console.log('metas:',this.ds_clasif);
        })
        .catch(function(error){
        console.log(error);
        })
      ...
      async kit_clasif(){
        try{
          let datos=await axios.get('http://localhost:3000/kit_clasif/'+this.cod_clf)
          console.log("cod clasificador",this.cod_clf);
          console.log(datos.data);
          this.ds_kit=await datos.data 
        }catch(error){
          console.log(error);
